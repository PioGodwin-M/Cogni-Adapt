...
var InferenceClientHttpRequestError = class extends InferenceClientError {
  ...
  constructor(message, httpRequest, httpResponse) {
    super(message);
    this.httpRequest = __spreadValues(__spreadValues({}, httpRequest), httpRequest.headers ? {
      headers: __spreadValues(__spreadValues({}, httpRequest.headers), "Authorization" in httpRequest.headers ? { Authorization: `Bearer [redacted]` } : void 0)
    } : void 0);
...
var BaseConversationalTask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    return __spreadProps(__spreadValues({}, params.args), {
      model: params.model
    });
  }
...
var HFInferenceAutomaticSpeechRecognitionTask = class extends HFInferenceTask {
  ...
  async preparePayloadAsync(args) {
    return "data" in args ? args : __spreadProps(__spreadValues({}, omit(args, "inputs")), {
      data: args.inputs
    });
  }
};
...
var BlackForestLabsTextToImageTask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    return __spreadProps(__spreadValues(__spreadValues({}, omit(params.args, ["inputs", "parameters"])), params.args.parameters), {
...
var FalAITextToImageTask = class extends FalAITask {
  preparePayload(params) {
    const payload = __spreadProps(__spreadValues(__spreadValues({}, omit(params.args, ["inputs", "parameters"])), params.args.parameters), {
      sync_mode: true,
      prompt: params.args.inputs
    });
...
var FalAIImageToImageTask = class extends FalAiQueueTask {
  ...
  async preparePayloadAsync(args) {
    ...
    return __spreadProps(__spreadValues(__spreadValues(__spreadProps(__spreadValues({}, omit(args, ["inputs", "parameters"])), {
...
var FalAIImageToVideoTask = class extends FalAiQueueTask {
  ...
  async preparePayloadAsync(args) {
    const mimeType = args.inputs instanceof Blob ? args.inputs.type : "image/png";
    return __spreadValues(__spreadValues(__spreadProps(__spreadValues({}, omit(args, ["inputs", "parameters"])), {
...
var FalAIAutomaticSpeechRecognitionTask = class extends FalAITask {
  ...
  async preparePayloadAsync(args) {
    ...
    return __spreadProps(__spreadValues({}, "data" in args ? omit(args, "data") : omit(args, "inputs")), {
...
var FalAIImageSegmentationTask = class extends FalAiQueueTask {
  ...
  async preparePayloadAsync(args) {
    ...
    return __spreadProps(__spreadValues(__spreadValues(__spreadValues({}, omit(args, ["inputs", "parameters", "data"])), args.parameters), args), {
...
var FeatherlessAITextGenerationTask = class extends BaseTextGenerationTask {
  ...
  preparePayload(params) {
    return __spreadProps(__spreadValues(__spreadValues({
      model: params.model
    }, omit(params.args, ["inputs", "parameters"])), params.args.parameters ? __spreadValues({
      max_tokens: params.args.parameters.max_new_tokens
    }, omit(params.args.parameters, "max_new_tokens")) : void 0), {
      prompt: params.args.inputs
    });
  }
  async getResponse(response) {
...
var HyperbolicTextGenerationTask = class extends BaseTextGenerationTask {
  ...
  preparePayload(params) {
    ...
    }, params.args.parameters ? __spreadValues({
      max_tokens: params.args.parameters.max_new_tokens
    }, omit(params.args.parameters, "max_new_tokens")) : void 0), omit(params.args, ["inputs", "parameters"])), {
      model: params.model
    });
  }
  async getResponse(response) {
...
var NovitaTextToVideoTask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    const _a = params.args.parameters ?? {}, { num_inference_steps } = _a, restParameters = __objRest(_a, ["num_inference_steps"]);
    return __spreadProps(__spreadValues(__spreadValues({}, omit(params.args, ["inputs", "parameters"])), restParameters), {
...
var ReplicateTask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    return {
      input: __spreadProps(__spreadValues(__spreadValues({}, omit(params.args, ["inputs", "parameters"])), params.args.parameters), {
        prompt: params.args.inputs
      }),
...
var WavespeedAITask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    const payload = __spreadProps(__spreadValues(__spreadValues({}, omit(params.args, ["inputs", "parameters"])), params.args.parameters ? omit(params.args.parameters, ["images"]) : void 0), {
      prompt: params.args.inputs
    });
...
var WavespeedAIImageToImageTask = class extends WavespeedAITask {
  ...
  async preparePayloadAsync(args) {
    ...
    return __spreadProps(__spreadValues({}, args), { inputs: args.parameters?.prompt, image: base, images });
  }
};
...
function makeRequestOptionsFromResolvedModel(resolvedModel, providerHelper, args, mapping, options) {
  const _a = args, { accessToken, endpointUrl, provider: maybeProvider, model } = _a, remainingArgs = __objRest(_a, ["accessToken", "endpointUrl", "provider", "model"]);
  void model;
  void maybeProvider;
  const provider = providerHelper.provider;
...
async function* innerStreamingRequest(args, providerHelper, options) {
  const { url, info } = await makeRequestOptions(__spreadProps(__spreadValues({}, args), { stream: true }), providerHelper, options);
  const response = await (options?.fetch ?? fetch)(url, info);
  if (options?.retry_on_error !== false && response.status === 503) {
...
async function audioClassification(args, options) {
  ...
  const { data: res } = await innerRequest(payload, providerHelper, __spreadProps(__spreadValues({}, options), {
...
async function textToSpeech(args, options) {
  ...
  const { data: res } = await innerRequest(args, providerHelper, __spreadProps(__spreadValues({}, options), {
    task: "text-to-speech"
  }));
  return providerHelper.getResponse(res);
}
...
function preparePayload2(args) {
  return "data" in args ? args : __spreadProps(__spreadValues({}, omit(args, "inputs")), { data: args.inputs });
}
...
async function imageSegmentation(args, options) {
  ...
  const { url, info } = await makeRequestOptions(args, providerHelper, __spreadProps(__spreadValues({}, options), { task: "image-segmentation" }));
...
async function imageToImage(args, options) {
  ...
  const { url, info } = await makeRequestOptions(args, providerHelper, __spreadProps(__spreadValues({}, options), { task: "image-to-image" }));
  return providerHelper.getResponse(res, url, info.headers);
}
...
async function imageToVideo(args, options) {
  ...
  const { url, info } = await makeRequestOptions(args, providerHelper, __spreadProps(__spreadValues({}, options), { task: "image-to-video" }));
  return providerHelper.getResponse(res, url, info.headers);
}
...
async function imageTextToImage(args, options) {
  ...
  const payload = await providerHelper.preparePayloadAsync(args);
  const { data: res, requestContext } = await innerRequest(payload, providerHelper, __spreadProps(__spreadValues({}, options), {
...
async function textToImage(args, options) {
  ...
  const { url, info } = await makeRequestOptions(args, providerHelper, __spreadProps(__spreadValues({}, options), { task: "text-to-image" }));
...
async function textToVideo(args, options) {
  ...
  const { data: response } = await innerRequest(args, providerHelper, __spreadProps(__spreadValues({}, options), {
    task: "text-to-video"
  }));
  const { url, info } = await makeRequestOptions(args, providerHelper, __spreadProps(__spreadValues({}, options), { task: "text-to-video" }));
...
async function* chatCompletionStream(args, options) {
  ...
  yield* innerStreamingRequest(args, providerHelper, __spreadProps(__spreadValues({}, options), {
    task: "conversational"
  }));
}
...
async function documentQuestionAnswering(args, options) {
  ...
  const { data: res } = await innerRequest(reqArgs, providerHelper, __spreadProps(__spreadValues({}, options), {
...
var InferenceClient = class _InferenceClient {
      ...
      Object.defineProperty(this, name, {
        ...
        value: (params, options) => (
          ...
          fn(
            ...
            __spreadValues({ endpointUrl: defaultOptions.endpointUrl, accessToken }, params),
            __spreadValues(__spreadValues({}, omit(defaultOptions, ["endpointUrl"])), options)
          )
        )
      });
    }
  ...
  endpoint(endpointUrl) {
    return new _InferenceClient(this.accessToken, __spreadProps(__spreadValues({}, this.defaultOptions), { endpointUrl }));
  }
};
...
function getData(type, partialTaskData = data_default21) {
  return __spreadProps(__spreadValues({}, partialTaskData), {
    id: type,
...
var loadTemplate = (language, client, templateName) => {
  ...
  return (data) => new Template(template).render(__spreadValues({}, data));
};
...
var snippetGenerator = (templateName, inputPreparationFn) => {
  return (model, provider, inferenceProviderMapping, opts) => {
    ...
    const request2 = makeRequestOptionsFromResolvedModel(providerModelId, providerHelper, __spreadValues({
      accessToken: accessTokenOrPlaceholder,
      provider,
      endpointUrl: opts?.endpointUrl ?? (provider === "auto" ? HF_ROUTER_AUTO_ENDPOINT : void 0)
    }, inputs), inferenceProviderMapping, {
      task,
      billTo: opts?.billTo
    });
    ...
    const autoInputs = !opts?.endpointUrl && !opts?.directRequest ? provider !== "auto" ? __spreadProps(__spreadValues({}, inputs), {
      model: `${model.id}:${provider}`
    }) : __spreadProps(__spreadValues({}, inputs), {
      model: `${model.id}`
      // if no :provider => auto
    }) : providerInputs;
    const params = {
    ...
    return inferenceSnippetLanguages.map((language) => {
      ...
      return langClients.map((client) => {
        ...
        if (client === "huggingface_hub") {
          const importSection = snippetImportPythonInferenceClient(__spreadValues({}, params));
          snippet = `${importSection}

${snippet}`;
        } else if (client === "requests") {
          const importSection = snippetImportRequests(__spreadProps(__spreadValues({}, params), {
...
var prepareConversationalInput = (model, opts) => {
  ...
  }, opts?.temperature ? { temperature: opts?.temperature } : void 0), opts?.max_tokens ? { max_tokens: opts?.max_tokens } : void 0), opts?.top_p ? { top_p: opts?.top_p } : void 0);
};
